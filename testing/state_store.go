...
func (sst *StateStoreTester) TestRequestIsolation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			router.GET(GenerateUniquePath("set"), func(ctx httpx.Context) error {
				// capturedContext = ctx
				...
				return ctx.Text(200, "Set OK")
			})

			// Second request - should not see the value from first request
			router.GET(GenerateUniquePath("get"), func(ctx httpx.Context) error {
				// capturedContext = ctx
