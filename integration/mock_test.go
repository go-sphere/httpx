...
// TestTestCasesCreation tests that TestCases can be created properly
func TestTestCasesCreation(t *testing.T) {
	engine := &MockEngine{}

	// Test creation without config
	tc := NewTestCases("mock", engine)
	if tc == nil {
...
// TestTestCasesWithConfig tests creation with custom config
func TestTestCasesWithConfig(t *testing.T) {
	engine := &MockEngine{}
	...
	tc := NewTestCasesWithConfig("mock", engine, config)
	if tc == nil {
		t.Fatal("TestCases should not be nil")
	}
